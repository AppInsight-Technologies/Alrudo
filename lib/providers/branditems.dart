...
class BrandItemsList with ChangeNotifier {
  ...
  Future<void> ReferEarn()async {
   ...
   //  try {
      ...
      print("Referal api..."+Api.getRefereal + (VxState.store as GroceStore).userData.branch.toString());
      print("Params..."+(VxState.store as GroceStore).userData.branch.toString());
      final responseJson = json.decode(utf8.decode(response.bodyBytes));
      print("Refer response..."+responseJson.toString());
      if (responseJson.toString() != "[]") {
        List data = [];
        /*responseJson.asMap().forEach((index, value) =>
          data.add(responseJson[index] as Map<String, dynamic>)
      );*/
        print("Refer image...."+responseJson['data']['image'].toString());
